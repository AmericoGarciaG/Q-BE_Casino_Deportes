...
* **Ω (Resumen):** Extracción determinista de tablas de posiciones completas (18 clubes), fixtures activos, métricas de tiros, posesión y goles esperados reales ($xG / xGA$) desde los endpoints JSON de FotMob.
* **I (Input):** `league_id` (e.g. `262` para Liga MX).
* **P (Process) [ARCH-PILLAR] [BIZ-LOGIC]:**
  1. Consultar endpoints REST de FotMob API (`leagues?id=262`, `matchDetails?matchId=...`).
  2. Parsear tabla general: posiciones, PJ, PG, PE, PP, GF, GC, diferencia de goles, puntos, forma reciente (5 partidos W/D/L) y xG acumulado por club.
  3. Parsear forma reciente (10P): goles anotados ($GF$), goles concedidos ($GC$), tiros a puerta ($SoT$), tiros permitidos ($SoTA$) y posesión ($Poss\%$).
  4. Extraer $xG$ promedio a favor y $xGA$ promedio en contra por partido.
  5. Cero simulación: si la API falla, activar `CUARENTENA_DATOS_INSUFICIENTES`; prohibida la fabricación de datos mockeados.
* **O (Output):** `FotMobLeagueSnapshot` validado contra Pydantic V2.
* **Φ (Transición):** Hacia **[LN-QBE-010]**.
* **[SHIELD]:** `tests/shield/abstract_test_LN_QBE_003_fotmob_provider.py`
* **[Binding Rationale]:** `[ARCH-PILLAR]` Datos deportivos oficiales respaldados por Opta con costo cero y latencia inferior a 200 ms.
